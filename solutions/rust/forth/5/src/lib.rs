...
use crate::instructions::{Definition, Instruction};
...
impl Forth {
	...
	fn parse_normal_word(&mut self, word: &str) -> Result<Instruction> {
		if word == ":" || word == ";" {
			...
			if let Some(call) = self.find_definition(&canonical) {
				Ok(call)
			} else {
	...
	fn parse_definition<'a>(&mut self, iterator: &mut impl Iterator<Item = &'a str>) -> ForthResult {
	...
	fn evaluate_instruction(&mut self, instr: Instruction) -> ForthResult {
		match instr {
			Instruction::Add => self.math(|a, b| Ok(a + b)),
			Instruction::Sub => self.math(|a, b| Ok(a - b)),
			Instruction::Mul => self.math(|a, b| Ok(a * b)),
			Instruction::Div => self.math(|a, b| if b == 0 {
	...
	fn math<F: FnOnce(Value, Value) -> Result<Value>>(&mut self, op: F) -> ForthResult {
		let rhs = self.pop()?;
		let lhs = self.pop()?;
	...
	fn find_definition(&self, word: &str) -> Option<Instruction> {
		for (index, definition) in self.dict.iter().enumerate().rev() {
