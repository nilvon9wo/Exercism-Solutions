...
public class TreeBuildingTests
{
	...
	{
		TreeBuildingRecord[] records = new[]
		{
			new TreeBuildingRecord { RecordId = 0, ParentId = 0 }
		};

		Tree tree = TreeBuilder.BuildTree(records);
	...
	{
		TreeBuildingRecord[] records = Array.Empty<TreeBuildingRecord>();

		_ = Assert.Throws<ArgumentException>(() => TreeBuilder.BuildTree(records));
	}
