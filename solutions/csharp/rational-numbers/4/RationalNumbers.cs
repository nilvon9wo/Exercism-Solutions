...
public struct RationalNumber
{
	...
	public RationalNumber Exprational(int power)
	{
		...
		if (power >= 0)
		{
			...
			powerNumerator = Math.Pow(doubleNumerator, doublePower);
			powerDenominator = Math.Pow(doubleDenominator, doublePower);
		}
		else
		{
			double positivePower = -power;
			powerNumerator = Math.Pow(doubleDenominator, positivePower);
			powerDenominator = Math.Pow(doubleNumerator, positivePower);
		}

		return new RationalNumber(Convert.ToInt32(powerNumerator), Convert.ToInt32(powerDenominator))
			.Reduce();
	}
