...
public static class SgfParser
{
	public static SgfTree ParseTree(string inputString)
	{
		...
		Result<Group>? root = GroupParser.Parse(input);
		return root.IsValid
			? root.Value.ToTree()
...
public class SgfTree
{
	...
	[SuppressMessage("Performance", "CA1819:Properties should not return arrays", Justification = "Test requirement.")]
...
internal static class KeyedOptionsParser
{
	...
	{
		Result<string> key = KeyParser.Parse(input);
		if (!key.IsValid)
		{
		...
		{
			Result<string> option = OptionParser.Parse(input);
			if (!option.IsValid)
			{
...
internal static class KeyParser
{
	...
	{
		Result<string> keyResult = input.TakeUntil(Symbols.OptionStart, "Key");
...
internal static class NodeFamilyParser
{
	...
	public static Result<NodeFamily> Parse(Input input)
	{
		...
		Result<Node> parentResult = ParseParent(input);
		if (!parentResult.IsValid)
		{
			return Result<NodeFamily>.From(parentResult.Exception);
		}

		Result<ReadOnlyCollection<Group>> childrenResults = ParseChildren(input);
		if (!childrenResults.IsValid)
		{
			return Result<NodeFamily>.From(childrenResults.Exception);
		}

		Node parent = parentResult.Value;
		ReadOnlyCollection<Group> children = childrenResults.Value;
		NodeFamily nodeFamily = !children.Any()
	...
	private static Result<Node> ParseParent(Input input)
	{
		...
		while (!IsEndOfOptions(input))
		{
			...
			KeyedOptions keyedOptionsValue = keyedOptions.Value;
	...
	private static Result<ReadOnlyCollection<Group>> ParseChildren(Input input)
	{
		...
		{
			Result<Group> childrenResult = GroupParser.Parse(input);
	...
	private static bool IsEndOfOptions(Input input)
		=> _optionEnding.Contains(input.Current);
}
...
{
	public static Result<string> Parse(Input input)
		=> input.Current != Symbols.OptionStart
...
public class Group
{
	public Group(IEnumerable<NodeFamily> nodes)
		=> _nodes = new ReadOnlyCollection<NodeFamily>(nodes.ToArray());

	private ReadOnlyCollection<NodeFamily> _nodes { get; init; }

	public SgfTree ToTree()
	{
		NodeFamily parentNodeFamily = _nodes[0];
		if (parentNodeFamily.Children?.Any() == true)
		{
			return parentNodeFamily.ToTree();
		}

		SgfTree[] children = _nodes
			.Skip(1)
...
{
	internal static bool ContainsLowerCase(this string value)
