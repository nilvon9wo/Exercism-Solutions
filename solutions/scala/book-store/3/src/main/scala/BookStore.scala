...
object BookStore {
  ...
  def total(bookIds: List[Int]): Int = {
    ...
    if (isNoDiscount(numberOfBooks, quantityByBooks)) {
      ...
      this.regroupTowardsMaximumDiscount(
  ...
  private def regroupTowardsMaximumDiscount(
    ...
    val groupedBySetSize: Map[Int, Iterable[Set[Int]]] = groupedTowardsSeriesCompletion.groupBy(_.size)
  ...
  private def regroupTowardsMaximumDiscount(
    ...
    regroupMap.put(4, newGroupsOf4 ++ groupedBySetSize.getOrElse(4, Nil))
