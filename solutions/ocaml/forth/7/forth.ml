...
let default_handler_by_token : (string, Context.t -> Context.t) Hashtbl.t =
  let handler_by_tokens = Hashtbl.create 16 in
  let add key fn = Hashtbl.replace handler_by_tokens (String.lowercase_ascii key) fn in
...
let shift_tokens_to_stack (context : Context.t) (tokens : string list ref) : string =
  ...
  Control_flow_helpers.do_while body cond;
  !next_operation
