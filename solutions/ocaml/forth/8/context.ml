...
let get_instruction (context : t) (word : string) : string option =
  let key = normalize_word word in
  try
    Some (Hashtbl.find context.instructions_by_word key)
  with Not_found ->
    None
...
let find_instruction (context : t) word =
    let key = normalize_word word in
    try Hashtbl.find context.instructions_by_word key
    with Not_found -> word

let add_word_to_builder (context : t) builder word =
    let instruction = find_instruction context word in
    let instruction_with_space = instruction ^ " " in
    Buffer.add_string builder instruction_with_space
