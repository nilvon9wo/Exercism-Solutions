...
let sum_first_n_rolls (rolls : int list) (count : int) : int =
  let sum = ref 0 in
  for roll_index = 0 to count - 1 do
    sum := !sum + get_roll_or_zero rolls roll_index
  done;
  !sum
...
let calculate_spare_bonus (context : Frame_context.t) : int =
  match Frame_context.get_next_frame context with
...
let calculate_strike_bonus (context : Frame_context.t) : int =
  ...
  | Some next_frame ->
    ...
    | _ ->
        sum_first_n_rolls (Frame.get_rolls next_frame) 2

let score_last_frame_spare (frame : Frame.frame) : int =
  (* The third roll is the bonus for spare in the last frame *)
  let rolls : int list = Frame.get_rolls frame in
  let bonus_roll : int =
    match List.nth rolls 2 with
    | Some bonus_roll_value -> bonus_roll_value
    | None -> 0
  in
...
let score_last_frame_strike (frame : Frame.frame) : int =
  ...
  let rolls : int list = Frame.get_rolls frame in
  let second_roll : int =
    match List.nth rolls 1 with
    | Some second_roll_value -> second_roll_value
    | None -> 0
  in
  let third_roll : int =
    match List.nth rolls 2 with
    | Some third_roll_value -> third_roll_value
    | None -> 0
  in
...
let score_frame (_self : t) (context : Frame_context.t) : int =
  ...
  if Frame.is_strike frame then
    if is_last_frame then
      score_last_frame_strike frame
    else
      score_strike frame context
  else if Frame.is_spare frame then
    if is_last_frame then
      score_last_frame_spare frame
    else
      score_spare frame context
  else
    Frame.pins_total frame
