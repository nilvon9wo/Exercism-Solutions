...
let add_roll (existing_frame : frame) (pins_knocked_down : int) : frame =
  { existing_frame with rolls = existing_frame.rolls @ [pins_knocked_down] }  (* or better: use List.rev_append for performance *)

let get_roll_or_zero (frame : frame) (roll_index : int) : int =
  if roll_index < List.length frame.rolls then
    List.nth_exn frame.rolls roll_index
  else
    0
...
let is_spare (frame : frame) : bool =
  ...
  not (is_strike frame) && total_pins_knocked_down = strike_pins_count
...
let pins_total (frame : frame) : int =
  List.sum (module Int) frame.rolls ~f:(fun pins_knocked_down -> pins_knocked_down)
