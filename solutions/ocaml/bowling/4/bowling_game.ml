...
let roll (bowling_game_state : bowling_game) (pins_knocked_down : int) : bowling_game =
  ...
  Roll_validator.validate_roll bowling_game_state.roll_validator (Roll_context.create current_frame pins_knocked_down);
  ...
  in
  { bowling_game_state with frames = ref updated_frames }


let score (bowling_game_state : bowling_game) : int =
  validate_game_is_complete bowling_game_state;
  Score_calculator.calculate_score bowling_game_state.score_calculator !(bowling_game_state.frames)
