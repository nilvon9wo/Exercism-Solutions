...
let rec propagate_value_change cell =
  match cell with
  | Input input_cell ->
      List.iter (fun dependent_cell ->
        match dependent_cell with
        | Compute dependent_compute_cell ->
            let updated_value = dependent_compute_cell.compute_fn () in
            if not (dependent_compute_cell.eq updated_value dependent_compute_cell.value) then begin
              dependent_compute_cell.value <- updated_value;
              List.iter (fun (_, callback_fn) -> callback_fn updated_value) dependent_compute_cell.registered_callbacks;
              propagate_value_change dependent_cell
            end
        | Input _ -> ()
      ) input_cell.dependent_cells
