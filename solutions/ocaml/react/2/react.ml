...
let set_value cell new_value =
  match cell with
  | Input c ->
      if c.eq new_value c.value then () else
      begin
        c.value <- new_value;
...
let create_compute_cell_1 dep ~f ~eq =
  ...
  let c = Compute { value = initial_value; eq; f = (fun () -> f (value_of dep)); callbacks = []; next_id = 0; dependents = [] } in
  begin match dep with
  | Input d -> d.dependents <- c :: d.dependents
  | Compute d -> d.dependents <- c :: d.dependents
  end;
  c

let create_compute_cell_2 dep1 dep2 ~f ~eq =
  let initial_value = f (value_of dep1) (value_of dep2) in
  let c = Compute { value = initial_value; eq; f = (fun () -> f (value_of dep1) (value_of dep2)); callbacks = []; next_id = 0; dependents = [] } in
