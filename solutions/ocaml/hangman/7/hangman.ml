...
let create secret_word =
  ...
  let masked_word_reactive = create_signal (Logic.generate_masked_word secret_word guessed_letters) in
  let progress_reactive = create_signal (let game_progress = Logic.evaluate_game_progress ~game_state:{
        secret_word;
        guessed_letters;
        remaining_failures;
...
let feed guessed_letter (api_game_state: t) =
  ...
  let updated_masked_word = Logic.generate_masked_word api_game_state.secret_word api_game_state.guessed_letters in
  ...
  let current_internal_progress = Logic.evaluate_game_progress ~game_state:refreshed_internal_state in
  let api_progress = convert_internal_progress_to_api current_internal_progress in
