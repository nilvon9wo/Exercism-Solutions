...
let create_initial_progress_signal
    ...
    let internal_progress = Logic.evaluate_game_progress ~game_state:{
...
let create secret_word =
    ...
    let progress_reactive = create_initial_progress_signal
            ~secret_word
            ~guessed_letters
            ~remaining_failures
            ~masked_word_reactive
    in
    {
        secret_word;
        guessed_letters;
...
let apply_guess_to_internal_state ~(api_game_state : t) ~(guessed_letter : char) =
...
let refresh_masked_word_signal ~(api_game_state : t) =
    let updated_masked_word =
        Logic.generate_masked_word ~secret_word:api_game_state.secret_word ~guessed_letters:api_game_state.guessed_letters
    in
    api_game_state.update_masked_word_signal updated_masked_word

let refresh_progress_signal ~(api_game_state : t) =
    let internal_game_state = convert_api_game_state_to_internal api_game_state in
    let internal_progress = Logic.evaluate_game_progress ~game_state:internal_game_state in
    let api_progress = convert_internal_progress_to_api internal_progress in
    api_game_state.update_progress_signal api_progress
