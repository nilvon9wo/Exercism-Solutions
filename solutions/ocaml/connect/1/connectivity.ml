...
let start_edge_coordinates (context : Context.t) : Coordinate.t list =
  ...
  | Player.X ->
      ...
      List.init (Board.height board) ~f:(fun row ->
          Board.coordinate_at_row_column board ~row ~column:0)
      |> List.filter ~f:(cell_belongs_to_player context)
  | Player.O ->
      (* O connects top to bottom: all coordinates in the top row *)
      List.init (Board.width board) ~f:(fun column ->
          Board.coordinate_at_row_column board ~row:0 ~column)
      |> List.filter ~f:(cell_belongs_to_player context)
...
let is_target_edge (context : Context.t) (coordinate : Coordinate.t) : bool =
  let board = context.board in
  match context.player with
  | Player.X -> Board.column_of_coordinate coordinate = Board.width board - 1
  | Player.O -> Board.row_of_coordinate coordinate = Board.height board - 1
