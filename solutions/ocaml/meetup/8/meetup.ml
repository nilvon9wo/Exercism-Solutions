...
let weekday_of_date ~(date: Date.t) =
  let year, month =
    if (date.month < 3)
...
let day_matches_weekday (weekday : Weekday.t) ~(date: Date.t) =
  weekday_of_date ~date = weekday_to_index weekday
...
let find_nth_weekday_in_month ~(date: Date.t) ~(weekday : Weekday.t) n =
  let rec loop day count =
    if day > days_in_month ~date
        ...
        else if day_matches_weekday weekday ~date:{date with day = Some day}
            then if count = n
                then day
...
let find_teenth_weekday ~date ~weekday =
  let rec loop day =
    if day_matches_weekday weekday ~date:{date with day = Some day}
        then day
        else loop (day + 1)
  in
  loop 13
...
let meetup_day input_schedule input_weekday ~year ~month =
  ...
  in
  let result = Date.make_full ~year ~month ~day in
