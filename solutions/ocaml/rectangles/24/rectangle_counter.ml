...
let count_rectangles_for_row_pair context row_pair_with_cols =
    let shared_columns = filter_shared_columns row_pair_with_cols.top.columns row_pair_with_cols.bottom.columns in
    let row_pair = Row_pair.create row_pair_with_cols.top.row_index row_pair_with_cols.bottom.row_index in
    count_rectangles_for_column_pairs context row_pair shared_columns

(* FIXME: too many parameters! *)
let accumulate_row_pair_counts context top_row_with_columns total remaining_row_data =
    let (bottom_row, bottom_columns) = remaining_row_data in
    let rectangle_count = count_rectangles_for_row_pair context {
        top = top_row_with_columns;
        bottom = {
