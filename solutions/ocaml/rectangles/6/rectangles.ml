...
let is_complete_rectangle (grid : grid) top_left bottom_right =
...
let count_rectangles_for_column_pairs grid top_row bottom_row corner_columns =
  let rec count_from_left accumulated_count = function
    ...
    | left_col :: remaining_cols ->
        let left_pos = {row = top_row; column = left_col} in
        let rights = List.map (fun col -> {row = bottom_row; column = col}) remaining_cols in
        let updated_count =
          count_from_right grid top_row bottom_row left_pos rights accumulated_count
        in
