...
let corner_columns_for_row (grid : grid) row_index =
    Fun.id
    |> List.init grid.column_count
    |> List.filter (fun column_index -> is_corner grid { row = row_index; column = column_index })
...
let is_complete_rectangle (grid : grid) top_left_position bottom_right_position =
    let rectangle = Rectangle.create top_left_position bottom_right_position in
    is_complete rectangle grid

let rec count_from_right (grid : grid) top_row bottom_row left_position remaining_right_positions accumulated_count =
...
let count_rectangles_for_column_pairs grid top_row bottom_row corner_columns =
    let rec count_from_left accumulated_count = function
        ...
        | left_column :: remaining_columns ->
            ...
            let right_positions = List.map (fun column_index -> { row = bottom_row; column = column_index }) remaining_columns in
...
let rec count_rectangles_across_rows grid accumulated_count = function
      ...
      | (top_row, top_row_columns) :: remaining_rows ->
          let updated_count =
            List.fold_left (fun running_total (bottom_row, bottom_row_columns) ->
