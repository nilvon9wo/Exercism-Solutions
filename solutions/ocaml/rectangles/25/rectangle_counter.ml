...
(* FIXME: too many parameters!
    FIXME:
       1. It should be possible to get `bottom_row_with_columns` from `(bottom_row, bottom_columns)` to
       2. It should be possible to get `row_pair_with_columns` from `top_row_with_columns` and `bottom_row_with_columns`
       3. Thus, we should be able to make this `let accumulate_row_pair_counts context total row_pair_with_columns =`
 *)
let accumulate_row_pair_counts context top_row_with_columns total (bottom_row, bottom_columns) =
    let row_pair_with_columns = Row_pair_with_columns.row_pair_with_columns_from_top_and_bottom top_row_with_columns bottom_row bottom_columns in
...
let rec count_rectangles_across_rows context accumulated_count = function
    ...
    | (top_row, top_columns) :: remaining_rows ->
        let top_row_with_columns = create top_row top_columns in
        (* FIXME: `fold_function` does not reveal the intention.
            Also, it feels like a nasty cheat so we don't see as many parameter-like things
            after 'updated_count` which has too many parameter like things anyway...
            ... but the split may hide things which can be combined.
            ... and right now, I can't understand where `remaining_row_data` above is coming
            from (`remaining_rows`? but if so, why can't we do
            `let (bottom_row, bottom_columns) = remaining_row_data in` here and then create
            `row_pair_with_columns` here?
         *)
        let fold_function = accumulate_row_pair_counts context top_row_with_columns in
