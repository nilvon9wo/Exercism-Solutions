...
(* --- Helpers --- *)
let compute_updated_count (input: counting_input) accumulated_count =
    if input.context.validator input.left_position input.right_position
...
let create_counting_input state right_position = {
       context = state.context;
       left_position = state.candidate.left_position;
       right_position
}
...
let rec count_from_right state accumulated_count =
  ...
  | right_position :: remaining_right_positions ->
      let input = create_counting_input state right_position in
      let updated_count = compute_updated_count input accumulated_count in
      let new_state = update_state_candidate_right_position state remaining_right_positions in
      count_from_right new_state updated_count

let update_state_candidate state new_candidate = {
      state with
...
let rec count_from_left context row_pair accumulated_count = function
  ...
  | left_column :: remaining_columns ->
      let counting_state = Counting_state.from_row_pair_and_columns context row_pair left_column remaining_columns in
