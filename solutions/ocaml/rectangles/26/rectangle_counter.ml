...
let rec count_rectangles_across_rows context accumulated_count = function
    ...
    | top_row_with_columns :: remaining_rows ->
        let bar = Row_pair_with_columns.create top_row_with_columns in
        let row_pairs_with_columns = List.map bar remaining_rows in
        let foo = accumulate_row_pair_counts context in
        let updated_count = List.fold_left foo accumulated_count row_pairs_with_columns in
