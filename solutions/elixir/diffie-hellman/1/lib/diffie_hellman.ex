defmodule DiffieHellman do
  ...
  def generate_public_key(prime_p, prime_g, private_key),
    do: find_remainder_of_power(prime_g, private_key, prime_p)
  ...
  def generate_shared_secret(prime_p, public_key_b, private_key_a),
    do: find_remainder_of_power(public_key_b, private_key_a, prime_p)

  def find_remainder_of_power(inital_value, power, divisor),
    do:
      inital_value
      |> Exponator.raise_to_power(power)
      |> Kernel.trunc()
      |> rem(divisor)
end

# @see: https://stackoverflow.com/questions/32024156/how-do-i-raise-a-number-to-a-power-in-elixir
defmodule Exponator do
  require Integer

  def raise_to_power(_, 0),
    do: 1

  def raise_to_power(x, exponent)
      when Integer.is_odd(exponent),
      do: x * raise_to_power(x, exponent - 1)

  def raise_to_power(x, exponent) do
    new_exponent = div(exponent, 2)
    raise_to_power(x, new_exponent) * raise_to_power(x, new_exponent)
  end
end
