...
defmodule Markdown.Headers do
  ...
  defp parse_header_markdown_level(header) do
    ...
    {header_level(level_indicator), Enum.join(content, " ")}
  end
  ...
  defp render_html_header({level_indicator, title}),
    do: "<h#{level_indicator}>#{title}</h#{level_indicator}>"
end
...
defmodule Markdown.Paragraphs do
  ...
  def to_html_paragraph(word_list),
    do: "<p>#{FontDecorator.join_words_with_tags(word_list)}</p>"
end
