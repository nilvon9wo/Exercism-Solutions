...
def generate_seats(number):
    seats_generated = 0
    for row_number in generate_row_numbers():
        for seat in generate_row_seats(row_number):
...
def generate_row_seats(row_number):
    for seat_letter in generate_seat_letters(SEATS_PER_ROW):
        yield f"{row_number}{seat_letter}"


def assign_seats(passengers):
    seats = zip(passengers, generate_seats(len(passengers)))
    return {
        passenger: seat
        for passenger, seat in seats
    }
